                                ...
                                @if ($m->status == 'active')
                                    <div class="carusel-item-content carusel-item-content-{{ $m->id }}" >
                                        <div class="box-item item item-{{ $key }}" style="border-top: 8px solid {{ $m->plan_calendar_color }}"  data-match-height="memberships-options">
                                            ...
                                            <h3 class="h3" style="color: {{ $m->plan_calendar_color }}">
                                                {{ $m->get_price()->price }}
                                                @if ($m->plan_period==7 || $m->plan_period==14)
                                                    {{\App\Http\Controllers\AppSettings::get_setting_value_by_name('finance_currency') }}/{{ $m->plan_period }}days
                                                @elseif($m->plan_period==30)
                                                    {{\App\Http\Controllers\AppSettings::get_setting_value_by_name('finance_currency') }}/month
                                                @elseif($m->plan_period==90 || $m->plan_period==180)
                                                    {{\App\Http\Controllers\AppSettings::get_setting_value_by_name('finance_currency') }}/{{ $m->plan_period/30 }}months
                                                @elseif ($m->plan_period==360)
                                                    {{\App\Http\Controllers\AppSettings::get_setting_value_by_name('finance_currency') }}/year
                                                @elseif($m->plan_period=-1)
                                                    {{\App\Http\Controllers\AppSettings::get_setting_value_by_name('finance_currency') }}/lifetime
                                                @endif
                                            </h3>
